...
void Tutorial21_RayTracing::UpdateTLAS()
{
    ...
    for (int i = 0; i < 200; ++i)
    {
        ...
        inst.Transform.SetTranslation(pos.x, -5.4f, pos.z);
    }
...
void Tutorial21_RayTracing::CreateSBT()
{
    ...
    for (int i = 0; i < NumSmallSpheres; ++i)
    {
        ...
        const char* shaderName =
            (distribution(generator) < 6) ? "SphereSolida" : 
            (distribution(generator) < 8) ? "SpherePrimaryHit" :                                     
                                            "SphereGlass";
...
void Tutorial21_RayTracing::UpdateUI()
{
    ...
    if (ImGui::Begin("Configuración de Esferas", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ImGui::Text("Control de Esferas Pequeñas");
        if (ImGui::SliderInt("Cantidad de Esferas", &m_NumScatterSpheres, 0, MaxSmallSpheres))
        {
            ...
            const float radiusMax = 15.0f;
            const float yBase     = -3.5f;

            for (int i = 0; i < m_NumScatterSpheres; ++i)
            {
                float angle  = (float)i / m_NumScatterSpheres * 2.0f * PI_F;
                float radius = radiusMin + (radiusMax - radiusMin) * (0.5f + 0.5f * sinf(i * 0.1f)); 
                float x = radius * cosf(angle);
                float z = radius * sinf(angle);
                float y = yBase + 0.5f * sinf(i * 0.3f); 

                m_ScatterSpherePositions.emplace_back(x, y, z);
                m_ScatterSphereNames.push_back("Esfera_" + std::to_string(i));
            }
        }

        ImGui::Text("Instrucciones: Use WASD para mover la cámara");
        ImGui::End();
    }
}
} 